...
오케스트레이터 가이드의 디스패치 게이트는 권장 베이스라인이 설치되고 활성화되어
있는지 확인합니다. 먼저 설치하시기를 강력히 권장합니다. 설치하지 않으면 그
게이트의 베이스라인 항목은 적용되지 않으며, 워크플로가 전제하는 리뷰와 검증
규율을 잃게 됩니다.
...
지정하거나 CLI 고유의 지침 파일 이름을 이 파일에 심볼릭 링크로 연결하십시오.

에이전트는 [SETUP.md](SETUP.md)의 인터뷰를 진행하고
`docs/env/ENVIRONMENT.md`를 작성합니다. 영어가 아닌 언어를 선택하면 아래 언어
절에서 설명하는 전환도 함께 수행합니다.

`docs/env/ENVIRONMENT.md`는 Git이 추적하는 파일이므로, 공개 포크에 커밋하면
프로젝트 URL과 저장소 범위가 함께 공개됩니다. 커밋 여부는 직접 판단하십시오.
